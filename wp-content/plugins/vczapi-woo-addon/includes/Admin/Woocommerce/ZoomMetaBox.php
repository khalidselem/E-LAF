 ...
 * @since   1.1.0
 * @package Codemanas\ZoomWooBookingAddon\Admin\Woo
 */
class ZoomMetaBox {
	...
	public function add_metabox() {
		add_meta_box( 'vczapi-woocommerce-integration-meta', __( 'WooCommerce Integration', 'vczapi-woo-addon' ), array(
			$this,
			'rendor_sidebox'
		), 'zoom-meetings', 'side' );
	...
	public function rendor_sidebox( $post ) {
        ...
        <div id="vczapi-zoom-metabox-content">
            <div class="form-content">
                <label><?php _e( 'Enable Purchase ?', 'vczapi-woo-addon' ); ?></label>
				...
				<?php if ( $product === false || ( ! empty( $product ) && $product->get_type() == 'zoom_meeting' ) ) { ?>
                    <p class="description"><?php _e( 'Enabling this will create a new WooCommerce product linked with this post which will be purchasable by customers.', 'vczapi-woo-addon' ); ?></p>
                    <p class="description" style="color:red;"><?php _e( 'NOTE: Avoid checking this option if you want to sell this meeting from your WooCommerce shop page.', 'vczapi-woo-addon' ); ?></p>
				<?php } else { ?>
                    <p class="description">
						<?php _e( 'This Meeting has been linked with product, to change pricing - please change product options. You can change time and host from this post though.', 'vczapi-woo-addon' ); ?>
                    </p>
				<?php } ?>
            </div>
			<?php if ( $product === false || ( ! empty( $product ) && $product->get_type() == 'zoom_meeting' ) ) { ?>
                <div class="form-content show-on-checked" <?php echo ! empty( $fields ) && ! empty( $fields['enable_woocommerce'] ) ? false : 'style="display: none;"'; ?>>
                    <label><?php _e( 'Regular Price', 'vczapi-woo-addon' ); ?></label>
			...
			if ( $product != false && ! empty( $fields['enable_woocommerce'] ) ) {
				?>
                <p>
                    <strong><?php echo _e( 'Product Linked with', 'vczapi-woo-addon' ); ?>: </strong><br><strong>ID #</strong><a target="_blank" href="<?php echo esc_url( get_edit_post_link( $product_id ) ); ?>" title="<?php echo esc_html( $product->get_title() ); ?>"><?php echo esc_html( $product->get_id() ); ?></a><br>
	...
	public function rendor_registrants( $post ) {
		...
		if ( ! empty( $order_ids ) ) {
            ...
            <table id="vczapi-wc-meeting-registrants-dtable" class="vczapi-wc-meeting-registrants-table vczapi-data-table">
                <thead>
                <tr>
                    <th><?php _e( 'Order', 'vczapi-woo-addon' ); ?> #</th>
                    <th><?php _e( 'Billing Email', 'vczapi-woo-addon' ); ?></th>
                    <th><?php _e( 'Billing First Name', 'vczapi-woo-addon' ); ?></th>
                    <th><?php _e( 'Billing Last Name', 'vczapi-woo-addon' ); ?></th>
                    <th><?php _e( 'Registration Date', 'vczapi-woo-addon' ); ?></th>
                </tr>
                </thead>
                <tbody>
				<?php
				foreach ( $order_ids as $order_id ) {
                    ...
                    <tr>
                        <td>
                            <a target="_blank" href="<?php echo $order->get_edit_order_url(); ?>"><?php _e( 'View Order', 'vczapi-woo-addon' ); ?></a>
                        </td>
			...
			echo "<p>" . __( 'No bookings for this meeting so far.', 'vczapi-woo-addon' ) . "</p>";
		}
	}
	...
	private function create_product( $post, $settings ) {
		...
		if ( ! empty( $old_product_id ) ) {
			...
			if ( $objProduct != false && $objProduct->get_type() === "zoom_meeting" ) {
