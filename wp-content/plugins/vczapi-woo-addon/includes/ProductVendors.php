...
//			add_action( 'show_user_profile', [ $this, 'user_profile_fields' ] );
			...
			add_filter( 'vczapi_bookings_hosts', [ $this, 'meeting_host_for_bookings' ] );
		}
	}
	 ...
	 * @return array|bool
	 */
	public function meeting_host_for_bookings( $hosts = [] ) {
		$user_id = get_current_user_id();
		$vendor  = \WC_Product_Vendors_Utils::is_vendor( $user_id );
		if ( ! $vendor ) {
			return $hosts;
		...
		$host_id = get_user_meta( $user_id, 'user_zoom_hostid', true );
		if ( ! empty( $host_id && array_key_exists( $host_id, $hosts ) ) ) {
			$hosts = [ $host_id => $hosts[ $host_id ] ];
		}
