        ...
        <div class="dpn-zvc-sidebar-box">
            <div class="join-links">
				<?php if ( ! empty( $product ) && $product->get_type() == 'zoom_meeting' ) { ?>
                    <a class="btn btn-join-link" href="<?php echo esc_url( wc_get_checkout_url() . '?add-to-cart=' . $product->get_id() ); ?>"><?php echo apply_filters( 'vczapi_buy_now_text', __( 'Buy Now for', 'vczapi-woo-addon' ) ) . ' ' . wc_price( $product->get_price() ); ?></a>
				<?php } else if ( ! empty( $product ) && $product->get_type() != 'zoom_meeting' ) {
					?>
                    <a class="btn btn-join-link" href="<?php echo esc_url( get_permalink( $product_id ) ); ?>"><?php echo apply_filters( 'vczapi_buy_now_text', __( 'Buy Now', 'vczapi-woo-addon' ) ); ?></a>
					<?php
				} else { ?>
                    <p><?php _e( 'This product is not valid for purchase.', 'vczapi-woo-addon' ); ?></p>
				<?php } ?>
            </div>
        </div>
		<?php
		...
		if ( $type ) {
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				if ( $cart_product_id == $product_id ) {
					wc_add_notice( sprintf( __( 'You have already added this meeting in your cart ! %s', 'vczapi-woo-addon' ), '<a href="' . wc_get_cart_url() . '" tabindex="1" class="button wc-forward">View cart</a>' ), 'error' );
					$passed = false;
					break;
				}
			}
	...
	public function cart_meeting_details( $name, $cart_item, $cart_item_key ) {
		...
		if ( ! empty( $post_id ) ) {
			...
			if ( ! empty( $zoom_details ) && is_object($zoom_details) ) {
				...
				$add_name = sprintf(
					'<p style="margin-top:10px;"><strong>' . __( 'Host', 'vczapi-woo-addon' ) . ':</strong><br>%s</p><p style="margin-top:10px;"><strong>' . __( 'Time', 'vczapi-woo-addon' ) . ':</strong><br>%s</p><p><strong>' . __( 'Timezone', 'vczapi-woo-addon' ) . ':</strong><br>%s</p>',
					$host_name,
					esc_html(  vczapi_dateConverter( $zoom_details->start_time, $zoom_details->timezone, 'F j, Y, g:i a', true ) ),
