...
$post_social_share_on = lawsight_get_opt( 'post_social_share_on', false );
$post_tags_on = lawsight_get_opt( 'post_tags_on', true );
$post_feature_image_on = lawsight_get_opt( 'post_feature_image_on', true );
$post_navigation_on = lawsight_get_opt( 'post_navigation_on', false );
?>
    ...
    <div class="entry-blog">
        <?php if (has_post_format('gallery')) : ?>
            <div class="entry-featured entry-gallery">
                <?php
                $light_box = lawsight_get_post_format_value('post-gallery-lightbox', '0');
                $gallery_list = explode(',', lawsight_get_post_format_value('post-gallery-images', ''));
        ...
        <?php elseif (has_post_format('quote')) : ?>
            <div class="entry-featured">
                <?php
                    $quote_text = lawsight_get_post_format_value('post-quote-cite', '');
        ...
        <?php elseif (has_post_format('link')) : ?>
            <div class="entry-featured">
                <?php
                    $link_pf = lawsight_get_post_format_value('post-link-url', '#');
        ...
        <?php elseif (has_post_format('video')) : ?>
            <div class="entry-featured">
                <div class="entry-video featured-active">
                    <?php
                    $video_url = lawsight_get_post_format_value('post-video-url', '#');
                    $video_file = lawsight_get_post_format_value('post-video-file', '');
                    $video_html = lawsight_get_post_format_value('post-video-html', '');
                    $video = '';
                    if (!empty($video_url)) {
                        global $wp_embed;
        ...
        <?php elseif (has_post_format('audio')) : ?>
            <div class="entry-featured">
                <?php
                    $audio_url = lawsight_get_post_format_value('post-audio-url', '#');
        ...
        <?php else :
            if (has_post_thumbnail() && $post_feature_image_on ) {
                echo '<div class="entry-featured"><div class="post-image">'; ?>
                    <?php the_post_thumbnail('lawsight-large'); ?>
                <?php echo '</div></div>';
            } ?>
        <?php endif; ?>
        <div class="entry-body">
            <?php lawsight_post_meta(); ?>
    ...
    <div class="entry-footer">
        <?php if($post_tags_on || $post_social_share_on) : ?>
            <div class="entry-content-bottom">
                <?php if($post_tags_on) :
                    lawsight_entry_tagged_in();
                endif;
                if($post_social_share_on) :
                    lawsight_socials_share_default(); 
                endif; ?>
            </div>
        <?php endif; ?>
        <?php if($post_navigation_on) { lawsight_post_nav_default(); } ?>
    </div>
</article><!-- #post -->